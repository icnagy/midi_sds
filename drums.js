...
$(function() {
  ...
  function sendMessage(){
    ...
    option.value = 110 + globalSampleNumber;
    option.innerText = "Wave " + globalSampleNumber;
    document.getElementById("wave_" + (globalSampleNumber+1)).appendChild(option);
    document.getElementById("note_" + (globalSampleNumber+1)).value = 2 + (globalSampleNumber * 2);
    document.getElementById("tune_" + (globalSampleNumber+1)).value = 0 - (globalSampleNumber * 2);
    globalSampleNumber++;
  }

  function requestDrumKits(){
    midi_out.send([0xF0, 0x00, 0x00, 0x1B, 0x02, 0x02, channel, dumpProgramCommandByte, patchNumber, 0xF7]);
  }

  function appendPortToList(port) {
    var option = document.createElement("option");
    option.value = port.id;
    option.innerText = port.type + "|" + port.manufacturer + "|" + port.name + "|" + port.version;
  ...
  function onMIDISuccess( midiAccess ) {
    ...
    midiAccess.outputs.forEach(appendPortToList);
    midiAccess.inputs.forEach(appendPortToList);
  ...
  function onMIDIMessage(event){
    if(event.data[0] == 0xF0 &&
      ...
      event.data[4] == 0x02 &&
      event.data[5] == 0x02 ){
      if(event.data[7] == 0x05) {
        console.log("Received Drum Kit Dump...");
  ...
  if (window.File && window.FileList && window.FileReader) {
    ...
    if (xhr.upload) {
      filedrag = document.getElementById("sample_target");
      // file drop
  ...
  document.getElementById("midi_channel_select").item(0).addEventListener("change", function(){
    if(midi){
      midi_out = midi.outputs.get(this.value);
    }
  });
